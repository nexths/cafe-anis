...
function createLinkHotspotElement(hotspot) {
    ...
    for (var i = 1; i <= 3; i++) {
      ...
      chevron.classList.add('chevron', 'chevron-' + i);
      container.appendChild(chevron);
    }

    var transformProperties = [ '-ms-transform', '-webkit-transform', 'transform' ];
    for (var i = 0; i < transformProperties.length; i++) {
      container.style[transformProperties[i]] = 'rotate(' + hotspot.rotation + 'rad)';
    }
...
// Configuração do Áudio de Fundo
  ...
  function playAudio() {
    if (bgMusic && bgMusic.paused) {
      bgMusic.play().then(function() {
        ...
        console.log("Aguardando interação para liberar o áudio.", error);
      });
    }
  }
  ...
  // Atalho da tecla P para capturar parâmetros atuais
  window.addEventListener('keydown', function(e) {
    if (e.key === 'p' || e.key === 'P') {
      var v = viewer.view();
      if (v) {
        var p = v.parameters();
        console.log('--- TARGET VIEW CAPTURADA ---');
        console.log(JSON.stringify({
          yaw: parseFloat(p.yaw.toFixed(4)),
          pitch: parseFloat(p.pitch.toFixed(4)),
          fov: parseFloat(p.fov.toFixed(4))
        }, null, 2));
      }
    }
  });
